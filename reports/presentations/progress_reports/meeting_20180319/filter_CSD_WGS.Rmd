...
sam <- gen %>%
  ...
  rename_all(
      funs(
        stringr::str_replace_all(., "Samtools.Stats_", "") %>%
...
fastqc <- gen %>%
  ...
  rename_all(
      funs(
        stringr::str_replace_all(., "FastQC_", "") %>%
        stringr::str_replace_all(., '\\.1', '')
      )
  )
...
clean_qc <- sam %>% 
  ...
  rename_all(
      funs(
        stringr::str_replace_all(., "percent(age)?", "%") %>%
        stringr::str_replace_all(., 'reads', '')
      )
  )
...
## DONE
* No need to filter PCR duplicates
* No need to realign indels (minimal to no effect)
...
res.pca <- PCA(clean_qc, graph = FALSE)

# Percentage of variance explained by each PC
scree <- fviz_screeplot(res.pca, ncp=10)

# Contribution of each variable to the PCs
contrib <- fviz_pca_var(res.pca, col.var="cos2",axes =c(1,2), repel = T, labelsize=3) +
...
corrplot(cor_var, method="color", type = "upper", diag=F)
